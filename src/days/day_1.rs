...
impl Part1 {
    ...
    pub fn calculate_distance(file_path: &str) -> u32 {
...
impl Part2 {
    pub fn calculate_similarity_score(file_path: &str) -> i32 {
        let (left_list, right_list) = Part1::parse_file(file_path);
        ...
        let similarity_score: i32 = left_list
            .iter().map(|&num| num * right_count.get(&num).unwrap_or(&0)).sum();
